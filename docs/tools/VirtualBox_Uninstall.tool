#!/bin/bash
# $Id: VirtualBox_Uninstall.tool 135976 2020-02-04 10:35:17Z bird $
## @file
...
if test -n "${my_files[*]}"  -o  -n "${my_directories[*]}"; then
    ...
    if test "$my_rc" -ne 0; then
        echo "An error occurred during 'sudo rm', there should be a message above. (rc=$my_rc)"
...
for kext in "${my_kexts[@]}"; do
    ...
    if test "$my_rc2" -ne 0; then
        echo "An error occurred during 'sudo /sbin/kextunload -m $kext', there should be a message above. (rc=$my_rc2)"
        test -x /usr/bin/sudo    || echo "warning: Cannot find /usr/bin/sudo or it's not an executable."
